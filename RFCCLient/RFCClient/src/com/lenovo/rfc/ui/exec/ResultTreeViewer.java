...
import com.lenovo.rfc.utils.test.ITestManager.OnTestSuiteListUpdateListener;
...
public class ResultTreeViewer implements OnTestSuiteListUpdateListener {
	...
	public ResultTreeViewer(Composite parent) {
		testManager = TestManager.instance();
		testManager.addOnTestSuiteListUpdateListener(this);
		...
		treeViewer.addTreeListener(new ITreeViewerListener() {
			...
			// 调用asyncExec()等展开（或收起）操作执行完毕后，再执行
			private void packColumns(final TreeViewer treeViewer) {
				treeViewer.getControl().getShell().getDisplay()
	...
	private void initData() {
		treeViewer.setContentProvider(new ITreeContentProvider() {
			...
			public Object[] getElements(Object inputElement) {
				if (inputElement instanceof List<?>) {
					List<TestSuite> testSuites = (List<TestSuite>) inputElement;
			...
			public Object[] getChildren(Object parentElement) {
				if (parentElement instanceof TestSuite) {
					TestSuite testSuite = (TestSuite) parentElement;
					if (testSuite.getTestList().size() != 0) {
						return testSuite.getTestList().toArray();
					}
				} else if (parentElement instanceof Test) {
					...
					Entry<String, List<String>> entry = (Entry<String, List<String>>) parentElement;
		...
		treeViewer.setInput(testManager.getTestSuite());
	}
		...
		tvc.setLabelProvider(new ColumnLabelProvider() {
			...
			public String getText(Object element) {
				if (element instanceof TestSuite) {
					...
					return "Local: " + testSuite.getLocal();
				} else if (element instanceof Test) {
					Test test = (Test) element;
					return test.getSpec();
